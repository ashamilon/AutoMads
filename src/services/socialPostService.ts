...
export async function publishScheduledPost(postId: string): Promise<void> {
  ...
  // Publish to Facebook
  if (shouldFb && pageAccessToken && pageId) {
    try {
      ...
      errors.push(`FB: ${msg}`);
      logger.error({ e: msg, postId }, "Facebook publish failed");
    }
  }

  // Publish to Instagram
  if (shouldIg && pageAccessToken && igConfig?.enabled && igConfig.igUserId) {
    try {
  ...
  if (shouldTiktok) {
    logger.info({ postId }, "TikTok publishing not yet configured — skipping");
  }

  const published = (fbPostId || igMediaId) && errors.length === 0;
